...
Функция HMAC(Знач Ключ, Знач СтрокаДанных, Знач ХешФункция) Экспорт
	...
	Для к = 1 По СтрДлина(СтрокаДанных) Цикл
		
		StringSHA1 = StringSHA1 + ПреобразоватьДесятичнуюСИВHex(КодСимвола(Сред(СтрокаДанных,к,1)));
		
	КонецЦикла;
	
	//4 Применим хэш-функцию SHA-1 к строке, полученной на прошлом шаге.
	StringSHA1     = SHA1(StringSHA1,ХешФункция);
	kResult     = StringSHA1;

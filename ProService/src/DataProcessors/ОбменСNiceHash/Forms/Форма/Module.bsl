...
Процедура ЗагрузитьКурсыНаСервере()
	//APIEndpoint = Константы.АдресAPIBitcoinmine.Получить();
	//
	...
	//			ЗаписьКурса.ВалютаБазовая = Справочники.Валюты.Bitcoin;
...
Процедура ЗагрузитьСтатистикуМайнингаНаСервере()
	...
	ВзаимодействиеСNiceHash.ЗагрузитьСтатистикуМайнера(Кошелек);
...
Процедура ПостроитьГрафикДоходности()
	...
	//КонецЦикла;
	СписокПар = Новый Массив;
	СписокПар.Добавить(Новый Структура("База,Котировка",Справочники.Валюты.Bitcoin,Справочники.Валюты.UnitedStatesDollar));
	...
	Для Каждого Пара Из СписокПар Цикл
		...
		СрезТекущегоКурса = РегистрыСведений.КурсыКриптоВалют.СрезПоследних(ДатаКурса,Новый Структура("ВалютаБазовая,ВалютаКотировки",Справочники.Валюты.Bitcoin,Справочники.Валюты.UnitedStatesDollar)); 
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Итерация = Итерация + 1;
			//Курс = ВыборкаДетальныеЗаписи.Курс;
			СрезКурса = РегистрыСведений.КурсыКриптоВалют.СрезПоследних(ВыборкаДетальныеЗаписи.Период,Новый Структура("ВалютаБазовая,ВалютаКотировки",Справочники.Валюты.Bitcoin,Справочники.Валюты.UnitedStatesDollar));
			...
			НакоплениеBTC = ВыборкаДетальныеЗаписи.Сумма;
			
			//Проверка наличия выплаты за промежуток времени
			Если ВыплатыНеВлияютНаГрафик Тогда
				НакоплениеBTC = НакоплениеBTC + ВыплаченноеНакоплениеBTC;
				
				Если НакоплениеBTC < ПредыдущееНакоплениеBTC Тогда //Произошла выплата
					...
					ЗапросВыплат.Текст = 
						...
						|	И ВыплатыКриптовалютПулом.Кошелек = &Кошелек
					...
					Пока ВыборкаВыплат.Следующий() Цикл
						ВыплаченноеНакоплениеBTC = ВыплаченноеНакоплениеBTC + ВыборкаВыплат.Сумма;
						НакоплениеBTC = НакоплениеBTC + ВыборкаВыплат.Сумма;
					КонецЦикла;
					//НакоплениеUSD = Курс * НакоплениеBTC;
				КонецЕсли;
			КонецЕсли;
			
			НакоплениеUSD = Курс * НакоплениеBTC;
			НакоплениеUSDПоТекущемуКурсу = ТекущийКурс * НакоплениеBTC;
			Если Итерация = 1 Тогда
				...
				ПредыдущееНакоплениеBTC = НакоплениеBTC;
				
				МинимальныйКурс = Курс;
				МаксимальныйКурс = МинимальныйКурс;
				
				КурсНаНачало = Курс;
				НакоплениеНаНачало = НакоплениеBTC;
			Иначе
				...
				Если Итерация = КоличествоЗаписей Тогда
					КурсНаКонец = Курс;
					НакоплениеНаКонец = НакоплениеBTC;
				КонецЕсли;
			...
			Если Итерация % Шаг < 1
				...
				Подсказка = //Формат(Точка.Значение) + ":" + Символы.ПС
							...
							+ ВалютаБазовая + ": " + Формат(НакоплениеBTC,"ЧДЦ=5") + Символы.ПС
				...
				ДоходBTCЗаПериод = НакоплениеBTC - ПредыдущееНакоплениеBTC;
				...
				ДоходBTCЗаПериодИтоговая = ДоходBTCЗаПериодИтоговая + ДоходBTCЗаПериод;
				
				Если Итерация > 1 Тогда
					Расшифровка = Формат(ТекущийКурс,"ЧРГ=''''") + " " + Пара.Котировка.Код + " * " + Формат(НакоплениеBTC,"ЧДЦ=5; ЧРГ=''''") + " " + Пара.База.Код + " - " + Формат(ПредыдущееНакоплениеUSDПоТекущемуКурсу,"ЧДЦ=2; ЧРГ=''''") + " " + Пара.Котировка.Код + Символы.ПС +
								  Формат(ТекущийКурс * НакоплениеBTC, "ЧДЦ=2; ЧРГ=''''") + " " + Пара.Котировка.Код + " - " + Формат(ПредыдущееНакоплениеUSDПоТекущемуКурсу,"ЧДЦ=2; ЧРГ=''''") + " " + Пара.Котировка.Код;
				Иначе
...
Процедура УстановитьНадписьКурсаНаПериод()
	СрезКурса = РегистрыСведений.КурсыКриптоВалют.СрезПоследних(ДатаКурса,Новый Структура("ВалютаБазовая,ВалютаКотировки",Справочники.Валюты.Bitcoin,Справочники.Валюты.UnitedStatesDollar));
	...
	ТекстКурс = "1 " + Справочники.Валюты.Bitcoin.Код + " = " + Формат(Курс,"ЧРГ=''''") + " " + Справочники.Валюты.UnitedStatesDollar.Код;

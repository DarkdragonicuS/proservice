...
Процедура ПостроитьГрафикДоходности()
	...
	Для Каждого Пара Из СписокПар Цикл
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			ЗапросКурса.Текст =
				"ВЫБРАТЬ
				|	МАКСИМУМ(КурсыКриптоВалют.Период) КАК Период
				|ПОМЕСТИТЬ Дата
				|ИЗ
				|	РегистрСведений.КурсыКриптоВалют КАК КурсыКриптоВалют
				|ГДЕ
				|	КурсыКриптоВалют.ВалютаБазовая = &ВалютаБазовая
				|	И КурсыКриптоВалют.ВалютаКотировки = &ВалютаКотировки
				|	И КурсыКриптоВалют.Период <= &Период
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ
				|	КурсыКриптоВалют.Курс,
				|	КурсыКриптоВалют.ВалютаБазовая,
				|	КурсыКриптоВалют.ВалютаКотировки
				|ИЗ
				|	РегистрСведений.КурсыКриптоВалют КАК КурсыКриптоВалют
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Дата КАК Дата
				|		ПО КурсыКриптоВалют.Период = Дата.Период
				|ГДЕ
				|	КурсыКриптоВалют.ВалютаБазовая = &ВалютаБазовая
				|	И КурсыКриптоВалют.ВалютаКотировки = &ВалютаКотировки";
			
			ЗапросКурса.УстановитьПараметр("ВалютаКотировки", Справочники.Валюты.UnitedStatesDollar);
			ЗапросКурса.УстановитьПараметр("ВалютаБазовая", Справочники.Валюты.Ether);
			ЗапросКурса.УстановитьПараметр("Период", ВыборкаДетальныеЗаписи.Период);
			
			РезультатЗапросаКурса = ЗапросКурса.Выполнить();
			
			СрезКурса = РезультатЗапросаКурса.Выгрузить();
